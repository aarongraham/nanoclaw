...
Runs every 5 min on the NanoClaw host. Emits a single JSON object to stdout:
  {"wakeAgent": true, "data": {"device": {...}}}  -> agent wakes with device context
  {"wakeAgent": false}                             -> agent stays asleep
"""
import argparse
import json
import logging
...
# Make lib/ importable whether run via `python3 poll.py` or as a module.
...
DEFAULT_LEDGER = "/home/aaron/projects/nanoclaw/data/new-device-intake.db"
DEFAULT_LOG = "/home/aaron/projects/nanoclaw/logs/new-device-intake.log"
...
def main(argv=None) -> int:
    ...
    # 2. Open ledger, enforce serialization invariant
    ...
    try:
        ...
        # 3. Build ledger state map for filter
        rows = conn.execute(
        ...
        # 5. Emit wake payload with device details
        _emit(
            {
                "wakeAgent": True,
                "data": {
