...
def _import_poll():
    # Ensure the skill root is on sys.path so `import poll` resolves to poll.py.
...
def test_pending_older_than_3_days_is_cleared_and_new_candidate_taken(tmp_path, monkeypatch):
    # Seed an OLD pending row (5 days ago) for one MAC; a different MAC qualifies fresh.
    ...
    with patch("lib.netalertx.get_devices", return_value=devs):
        # Freeze "now" to 1_700_000_000 so the old pending is ~5 days stale
    ...
    # Old pending row should be cleared (pending=0) and new one set
    conn = ledger.open_db(ledger_path)
