...
def test_rejects_when_device_not_reconnected_since_last_ask():
    # 4 days elapsed, so the 3-day gate passes — but devLastConnection is much
    # older than last_asked_at, meaning the device hasn't been seen since we
    # asked. Per spec: don't re-ping.
    ...
    # now is 4 days after last_asked_at; devLastConnection is way before last_asked_at
    assert result is None


def test_accepts_when_3_days_and_reconnected():
    # last_asked_at=4 days ago, devLastConnection=yesterday → reconnected since
    four_days_ago = 1700000000 - 4 * 24 * 3600
    one_day_ago_ts = 1700000000 - 24 * 3600
    # Convert one_day_ago_ts to NetAlertX string format
