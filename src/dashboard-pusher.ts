...
function collectAgentGroups() {
  return getAllAgentGroups().map((g) => {
    ...
    // v2 stores container config as a JSON file at groups/<folder>/container.json
    // (not a DB column). Read it off disk if present.
    let containerConfig: unknown = null;
    try {
      containerConfig = readContainerConfig(g.folder);
    } catch {
      /* keep null */
    }
